...
from .stdin_server import StdinCommandServer
...
async def run_application(app, config):
    """Run the HTTP server and background STDIN command processor."""
    loop = asyncio.get_running_loop()
    stdin_server = StdinCommandServer(loop, lambda: config)
...
def main():
    ...
    try:
        ...
        config = load_config()
        logger.info(
            f"Loaded configuration: save_to={config.save_to}, "
        ...
        # Start HTTP and STDIN servers
        asyncio.run(run_application(app, config))
