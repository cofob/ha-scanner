...
class StdinCommandServer:
    ...
    def __init__(
        self, loop: asyncio.AbstractEventLoop, config_provider: Callable[[], AppConfig]
    ):
        self._loop = loop
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(
            target=self._reader, name="stdin-command-server", daemon=True
        )
        self._thread.start()
    ...
    def _reader(self) -> None:
        ...
        while self._running:
            ...
            if not line:
                logger.warning(
                    "STDIN input stream closed; continuing without STDIN commands"
                )
                break
            ...
            future = asyncio.run_coroutine_threadsafe(
                self._handle_payload(payload), self._loop
            )
            try:
                future.result()
    ...
    async def _handle_payload(self, raw_payload: str) -> None:
        ...
        if command == "list_devices":
            ...
            self._send_response(
                {"success": False, "error": "unknown_command", "command": command}
            )
    ...
    async def _handle_list_devices(self) -> None:
        ...
        try:
            ...
            self._send_response(
                {"success": False, "error": "scanner_error", "detail": str(exc)}
            )
    ...
    async def _handle_scan(self, overrides_data: Dict[str, Any]) -> None:
        ...
        if config is None:
            ...
            self._send_response(
                {"success": False, "error": "config_missing", "request_id": request_id}
            )
            return
        ...
        try:
            ...
            logger.info(
                f"[{request_id}] Saved {len(saved_files)} file(s): {saved_paths}"
            )

            telegram_result: Optional[Dict[str, Any]] = None
            should_send = (
                overrides.send_to_telegram
                if overrides and overrides.send_to_telegram is not None
            ...
            if should_send:
                if not config.telegram.bot_token or not config.telegram.chat_id:
                    ...
                    caption = (
                        overrides.caption
                        if overrides and overrides.caption
                        else config.telegram.caption
                    )
