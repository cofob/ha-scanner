...
async def send_document(
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        try:
            ...
            logger.error(f"HTTP error sending to Telegram: {e.response.status_code} - {e.response.text}")
...
async def send_media_group(
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        try:
            ...
            logger.info(f"Sent media group with {len(file_paths)} files to Telegram chat {chat_id}")
            return result['result']
            
        except httpx.HTTPStatusError as e:
            logger.error(f"HTTP error sending media group to Telegram: {e.response.status_code} - {e.response.text}")
            raise TelegramError(f"HTTP {e.response.status_code}: {e.response.text}")
        except Exception as e:
    ...
    try:
        ...
        if as_media_group and len(files) > 1:
            ...
            for file_path in files:
                message = asyncio.run(send_document(bot_token, chat_id, file_path, caption if len(files) == 1 else None))
